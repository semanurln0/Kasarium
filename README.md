# Kasarium — Modern Retail POS + Online Shop

**Your all-in-one retail solution for cash registers and online sales.**
...
Technical docs, setup instructions, and deployment guides live in `docs/FORDEVS.md`. See that file for contributing, testing, and running the project locally.
...
### User Roles
- **Owner (Superuser):** Full system control
- **Manager (Admin):** Daily operations and reporting
- **Staff:** POS and inventory tasks
- **Customer:** Browse and order online
...
- Tax: 21% VAT; prices displayed are tax-included; configurable by Admin
...
- Refund policy: Default 7 days; configurable by Admin
- Refund reasons: Selectable list + “Other” with free text
- Purchase price: Visible to Admin only
- Store info (name/address/VAT ID): Editable by Admin; appears on receipts
...
- Apps:
  - users: Custom User with `user_type = Admin | staff | customer`; register/login/logout; role-based access
...
### Superuser (Owner / You)
Full system control. Intended for the project owner only.
Access:
- Everything in the system (bypasses all permissions).
- Full Django Admin access to all apps/models.
- User/role management: create/edit/delete users, reset passwords, manage groups/permissions.
- Full control over products, categories, pricing, discounts, taxes, inventory, expiration dates.
- Full control over sales: create/edit/cancel/refund, override rules, view all transactions.
- Full reporting access: revenue, inventory, waste/expiry reports, exports.
- System settings: integrations, configuration, backups/maintenance tasks.

### Admin (Store Manager)
Manages daily operations but not system-level security.
Access:
- Manage products, categories, pricing, promotions/discount rules.
- Manage inventory and expiration records.
- Manage orders/sales, refunds/cancellations (optionally require approval for large amounts).
- View reports and export data (sales, inventory, expirations).
- Create/edit staff accounts (optional), but cannot manage permissions/groups unless explicitly allowed.
Restrictions:
- No access to low-level system configuration, permissions/groups (unless granted).
- Cannot perform superuser-only maintenance operations.

### Staff (Employees / Cashiers / Inventory Staff)
Handles day-to-day tasks with limited scope.
Access (depending on assignment):
- POS operations: create sales, scan barcodes, apply allowed discounts, print receipts/labels.
- Inventory updates: stock adjustments, receiving, expiration entry (if permitted).
- View-only access to product info and stock.
Restrictions:
- No user/role management.
- No global settings.
- Limited reporting (usually personal shift/day summaries only).

### Customer (End User)
Customer-facing experience (Phase 3+ / webshop).
Access:
- Browse catalog, search products.
- View product details (including EN translations if available).
- Place orders (if enabled), view own order history.
Restrictions:
- No admin/POS features.
- No access to internal inventory/expiration data.
...
- Product:
  ...
  - `purchase_price` (Admin-only visibility)
...
- StoreSetting:
  - Store name, address, phone, VAT ID/company ID (editable by Admin); receipt header/footer text; paper width presets
- Order:
...
- Refund:
  ...
  - `policy_days` default 7 (configurable by Admin)
- Register:
...
- Admin can set max discount thresholds (optional future feature)
...
- Store info from `StoreSetting` (editable by Admin)
- Content:
  ...
  - Summary:
    ...
    - Total payment
    - Payment method
  - Footer: Thank you for shopping! , refund policy (default 7 days)
- Printing approach:
...
- Messages are stored in the main database and visible to Admin
